...
class OhlcvAggregator {
  ...
  async init() {
    ...
    //  - else: create data set with start time = toBeSavedTradeEvents[0].executedTime and put all toBeSavedTradeEvents
    // to this data set else: there is market data for this time resolution (tmp_lastStartTime != null) create new data
    // set with start time is next start time of executed time of last saved trade set last close price for data set -
    // if no new trade event (!toBeSavedTradeEvents || toBeSavedTradeEvents.length = 0): some thing error! (how was
    // last market data saved?)  // - else (there are some unprocessed trade events): put all toBeSavedTradeEvents to
    // this data set
