...
const handleMessage = (event) => {
  ...
  switch(event.type){
    ...
    case OrderBookEvent.GET_OHCLV_DATA_POINTS:

      const {currency, baseCurrency, resolution, from, to, id, type} = event;

      const arrayOfDataPointsPromise = getDataPoints(currency, baseCurrency, resolution, from, to);

      arrayOfDataPointsPromise.then((arrayOfDataPoints) => {
